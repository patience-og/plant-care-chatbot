...
# --- 1. DATA PREPROCESSING FUNCTIONS (DEFINED FIRST) ---

def split_sentences(text):
    """Splits text into sentences using basic punctuation."""
    # Splits sentences by . ! ? followed by a space
    sentences = re.split(r'(?<=[.!?])\s+', text.strip())
    # Filter out empty strings and ensure no newlines remain
...
# --- 2. CHATBOT CORE FUNCTION (CALLS PREPROCESS) ---
...
def plant_chatbot(user_query, top_n=3):
    ...
    try:
        ...
        return "Error: Cannot find 'plant.txt'. Please check the file name and location."

    # Call preprocess (defined above)
    cleaned, original_sentences = preprocess(text)

    # Filter out sentences that are questions to avoid asking a question back
    statement_sentences = []
    cleaned_statements = []
    for i, s in enumerate(original_sentences):
        # A statement is a sentence that does not end with '?' and is not empty
    ...
    if similarity.max() < 0.15:
        return "I'm sorry, I couldn't find any relevant information for that query in my plant knowledge base."
    ...
    for i in top_indices:
        ...
        if similarity[i] > 0.05:
            # Add the original, uncleaned sentence
            response_parts.append(statement_sentences[i])

    if not response_parts:
         return "I couldn't find a sufficiently relevant answer in the data."

    # Join the top sentences for a rich answer
    # Add an empty line between sentences for clear formatting in Streamlit
    return "\n\n".join(response_parts)